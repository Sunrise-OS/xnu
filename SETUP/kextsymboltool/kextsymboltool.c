...
static void uuid_generate(uuid_t uuid) {
  ssize_t count = getrandom(uuid, sizeof(uuid_t), 0);
 ...
 * Function Protos
 *********************************************************************/
__private_extern__ ToolError readFile(const char *path, vm_offset_t *objAddr,
                                      vm_size_t *objSize);

__private_extern__ ToolError writeFile(int fd, const void *data, size_t length);

__private_extern__ ToolError seekFile(int fd, off_t offset);
...
/*********************************************************************
*********************************************************************/
__private_extern__ ToolError writeFile(int fd, const void *data,
                                       size_t length) {
  ToolError err;
...
__private_extern__ ToolError readFile(const char *path, vm_offset_t *objAddr,
  ...
  *objSize = 0;
  ...
  do {
    ...
    *objAddr = (vm_offset_t)mmap(
        NULL /* address */, *objSize, PROT_READ | PROT_WRITE,
        MAP_FILE | MAP_PRIVATE /* flags */, fd, 0 /* offset */);
 ...
 */
static int qsort_cmp(const void *_left, const void *_right) {
...
static int bsearch_cmp(const void *_key, const void *_cmp) {
  char *key = (char *)_key;
...
static int bsearch_cmp_prefix(const void *_key, const void *_cmp) {
...
static uint32_t count_symbols(char *file, vm_size_t file_size) {
  uint32_t nsyms = 0;
  char *scan;
  char *eol;
  char *next;
...
static uint32_t store_symbols(char *file, vm_size_t file_size,
                              struct symbol *symbols, uint32_t idx,
...
static const NXArchInfo *lookup_arch(const char *archstring) {
  /*
  ...
  static const NXArchInfo archlist[] = {
      {"x86_64", 0x01000007 /* CPU_TYPE_X86_64 */,
       3 /* CPU_SUBTYPE_X86_64_ALL */, NX_LittleEndian, NULL},
      {"x86_64h", 0x01000007 /* CPU_TYPE_X86_64 */,
       8 /* CPU_SUBTYPE_X86_64_H */, NX_LittleEndian, NULL},
      {"armv7", 12 /* CPU_TYPE_ARM */, 9 /* CPU_SUBTYPE_ARM_V7 */,
       NX_LittleEndian, NULL},
      {"armv7s", 12 /* CPU_TYPE_ARM */, 11 /* CPU_SUBTYPE_ARM_V7S */,
       NX_LittleEndian, NULL},
      {"armv7k", 12 /* CPU_TYPE_ARM */, 12 /* CPU_SUBTYPE_ARM_V7K */,
       NX_LittleEndian, NULL},
      {"arm64", 0x0100000c /* CPU_TYPE_ARM64 */, 0 /* CPU_SUBTYPE_ARM64_ALL */,
       NX_LittleEndian, NULL},
      {"arm64e", 0x0100000c /* CPU_TYPE_ARM64 */, 2 /* CPU_SUBTYPE_ARM64_E */,
       NX_LittleEndian, NULL},
  };
  unsigned long i;
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i += 2) {
    ...
    err = readFile(argv[i + 1], &files[num_files].mapped,
                   &files[num_files].mapped_size);
    if (kErrorNone != err) {
      exit(1);
    }
  ...
  if (num_import_syms) {
    for (export_idx = 0; export_idx < num_export_syms; export_idx++) {
      ...
      if (wild) {
        ...
        key.name = name;
        key.name_len = len;
        result = bsearch(&key, import_symbols, num_import_syms,
                         sizeof(struct symbol), &bsearch_cmp_prefix);
        ...
        result = bsearch(name, import_symbols, num_import_syms,
                         sizeof(struct symbol), &bsearch_cmp);
      }

      if (!result && require_imports) {
        int status;
        char *demangled_result = __cxa_demangle(
            export_symbols[export_idx].name + 1, NULL, NULL, &status);
        fprintf(stderr, "exported name not in import list: %s\n",
                demangled_result ? demangled_result
      ...
      if (result && !wild) {
        export_symbols[export_idx].flags |= kExported;
        strtabsize += (export_symbols[export_idx].name_len +
  ...
  if (CPU_ARCH_ABI64 & target_arch->cputype) {
    ...
    symcmd.stroff = result_count * sizeof(struct nlist_64) + symcmd.symoff;
    ...
    symcmd.symoff = symsoffset;
    symcmd.stroff = result_count * sizeof(struct nlist) + symcmd.symoff;
  ...
  for (export_idx = 0; export_idx < num_export_syms; export_idx++) {
    ...
    if (export_idx && export_symbols[export_idx - 1].name &&
        !strcmp(export_symbols[export_idx - 1].name,
                export_symbols[export_idx].name)) {
    ...
    for (import_idx = 0; import_idx < export_symbols[export_idx].list_count;
         import_idx++) {
  ...
  for (export_idx = 0; export_idx < num_export_syms; export_idx++) {
    ...
    for (import_idx = 0; import_idx < export_symbols[export_idx].list_count;
      ...
      if (export_symbols[export_idx].list[import_idx].indirect) {
        err =
            writeFile(fd, export_symbols[export_idx].list[import_idx].indirect,
